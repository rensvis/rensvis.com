...
date: 2026-03-08
tags: [angular, web-components, design-systems]
featured: false
---

If you're an Angular developer and you've used web components before, you've probably wrapped them in an Angular component. It's an established pattern and by now the standard approach. 

Let's explore an alternative: a directive-based adapter.
...
1. [Why We Use Wrapper Components](#why-we-use-wrapper-components)
2. [Hidden Drawbacks](#hidden-drawbacks)
   - [Drawback 1: Styling Friction](#drawback-1-styling-friction)
   - [Drawback 2: Slot-based Composition Can Break](#drawback-2-slot-based-composition-can-break)
   - [Drawback 3: API Drift](#drawback-3-api-drift)
3. [Alternative: Directive-Based Adapter](#alternative-directive-based-adapter)
   - [What You Gain](#what-you-gain)
4. [Which One Should You Use?](#which-one-should-you-use)
   - [API Purity vs API Control](#api-purity-vs-api-control)
   - [Design System Governance](#design-system-governance)
   - [Tech Debt: Where Does It Accumulate?](#tech-debt-where-does-it-accumulate)
   - [Control & Orchestration](#control--orchestration)
...
Great, now the compiler stops complaining. The problem however is that it stops checking our actual angular components too. This means that we can introduce bugs in our angular components that the compiler doesn't catch.
...
You either have to move the elements in the dom with portals (works, but complex), or you have to reshape the API so the wrapper takes an array and creates the elements itself. Either way, you're adding more complexity and documentation to the component.

### Drawback 3: API Drift

With components you have full control over the API. You can reshape and patch, which is handy. 

The flip side: it can **hide problems** instead of resolving them. Wrappers let you work around weaknesses in the design system, so the pressure to fix things at the source can fade. 

Over time, the Angular API and the web component API can drift apart. Multiple apps with their own wrappers might diverge in different directions. 
...
In my view, there are a few things worth considering when choosing between wrapper components and directives.
...
- **Directives** — The web component API is exposed directly. No reshaping, no compensation. If the API is awkward, consumers feel it — and so does the design system team. That can be a good thing if you're able to improve the design system; it can also be painful if you're not. A clear prefix (`ds-`) tells devs they're binding to a web component. Custom events, attribute vs property binding — platform behavior, not typical Angular. Worth considering if your team is comfortable with that.
...
Neither approach magically avoids debt. It just lands in different places. The directive approach could have the added benefit of forcing the design system team to improve the API.
...
- **Directives** — You test against the real element. It's closer to integration testing — less 'fake' abstraction. The compiler and runtime stay aligned. Changes to the web component show up in tests right away. Not a deciding factor on its own, but worth thinking about when test strategy and maintenance matter.
...
Which one fits depends on your context. If you need orchestration, or you're consuming a design system you can't change, wrappers might be the better fit. If you own or influence the design system, directives can work well — they keep the API honest and push responsibility to the source. There's no universal answer.
