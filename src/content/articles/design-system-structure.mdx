...
In enterprise systems, classification debates inevitably show up. Is this a molecule or an organism? Can an organism depend on another organism? Should templates contain logic? These discussions are rarely productive. They're usually a signal that the structure isn't aligned with responsibility.
...
Functional layering organizes by responsibility: tokens, primitives, smart components, layout, and content.

```
components/
├── tokens/
├── primitives/              # Purely visual
...
Atomic Design makes sense as a principle. It just isn't enough as an architecture.
...
Structure is not taxonomy.

Structure is power distribution.
